...
public class BookDAO {
    ...
    public ArrayList<Book> getAllBooks() {
        ...
        try {
            cn = DBUtils.getConnection();
            if (cn != null) {
                String sql = "SELECT id, title, author, category, isbn, published_year, total_copies, available_copies, status, url FROM books";
                PreparedStatement ps = cn.prepareStatement(sql);
                ResultSet rs = ps.executeQuery();
                while (rs.next()) {
                    result.add(new Book(
                            ...
                            rs.getInt("published_year"), rs.getInt("total_copies"),
                            rs.getInt("available_copies"), rs.getString("status"), rs.getString("url")));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ArrayList<Book> getListBookByCategory(String category) {
        ArrayList<Book> result = new ArrayList<>();
        Connection cn = null;
        try {
            cn = DBUtils.getConnection();
            if (cn != null) {
                String sql = "SELECT id, title, author, isbn, category, published_year, total_copies, available_copies, status, url "
                        + "FROM books WHERE category LIKE ?";
                PreparedStatement ps = cn.prepareStatement(sql);
